...
void AnchorManager::addStitch(VectorId anchor_id, RegionId region, LeaseHandle lease) {
	...
	if (!already_stitched) stitches.push_back(Stitch{region, lease});
}

LeaseHandle AnchorManager::removeStitch(VectorId anchor_id, RegionId region) {
	std::lock_guard<std::mutex> lock(mutex_);
	auto map_it = stitches_.find(anchor_id);
	if (map_it == stitches_.end()) return LeaseHandle{};

	std::vector<Stitch>& stitches = map_it->second;
	auto it = std::find_if(stitches.begin(), stitches.end(),
													[region](const Stitch& s) { return s.region == region; });
	if (it == stitches.end()) return LeaseHandle{};

	LeaseHandle lease = it->lease;
	stitches.erase(it);
