...
1. **Interface** -- `Index`, `Engine`. `Index` is the abstract entry point
   ...
   currently only) `Index` implementation. It owns an `IndexAdapter` and a
...
2. **Core** -- `Core`, `AnchorManager`. `Core` is the index-agnostic
   ...
   `IndexAdapter`/`IRegion` interfaces and the `RoutingCache` interface --
   never against a concrete index or a concrete cache. It holds:
   - `IndexAdapter& adapter_` -- injected, not owned.
   ...
   no knowledge of `RoutingCache`, `IndexAdapter`, or anything above it --
   it only understands `VectorId -> Stitch` bookkeeping.

3. **Adapter** -- `IndexAdapter`, `IRegion` (interfaces only; no concrete
