...
// heap-based search algorithms (and this file's copied one, hnsw_index_dist.cpp)
// rely on: lower value always means closer, for every metric.
//
// Cosine is not covered -- hnswlib has no native Cosine Space (see
// hnsw_index.cpp's makeEngine() TODO); LaunchDistanceKernel() below throws
...
__global__ void FloatAccumDistanceKernel(const ElemT* query, const ElemT* const* candidate_ptrs, std::size_t count,
																					std::uint32_t dim, float* out) {
...
template <typename ElemT, bool IsInnerProduct>
__global__ void IntAccumDistanceKernel(const ElemT* query, const ElemT* const* candidate_ptrs, std::size_t count,
																				std::uint32_t dim, float* out) {
	std::size_t candidate_index = blockIdx.x;
	if (candidate_index >= count) return;
	const ElemT* candidate = candidate_ptrs[candidate_index];
...
void LaunchDistanceKernel(KernelElemType dtype, KernelDistanceOp metric, const void* query, const void* const* candidate_ptrs,
	...
	switch (dtype) {
		case KernelElemType::Float32: {
			...
			if (is_inner_product) {
				FloatAccumDistanceKernel<float, true><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			} else {
				FloatAccumDistanceKernel<float, false><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			}
			break;
		}
		case KernelElemType::Float16: {
			const auto* q = static_cast<const std::uint16_t*>(query);
			const auto* const* c = reinterpret_cast<const std::uint16_t* const*>(candidate_ptrs);
			if (is_inner_product) {
				FloatAccumDistanceKernel<std::uint16_t, true><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			} else {
				FloatAccumDistanceKernel<std::uint16_t, false><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			}
			break;
		}
		case KernelElemType::UInt8: {
			const auto* q = static_cast<const unsigned char*>(query);
			const auto* const* c = reinterpret_cast<const unsigned char* const*>(candidate_ptrs);
			if (is_inner_product) {
				IntAccumDistanceKernel<unsigned char, true><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			} else {
				IntAccumDistanceKernel<unsigned char, false><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			}
			break;
		}
		case KernelElemType::Int8: {
			const auto* q = static_cast<const std::int8_t*>(query);
			const auto* const* c = reinterpret_cast<const std::int8_t* const*>(candidate_ptrs);
			if (is_inner_product) {
				IntAccumDistanceKernel<std::int8_t, true><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			} else {
				IntAccumDistanceKernel<std::int8_t, false><<<grid, block, 0, stream>>>(q, c, count, dim, out);
			}
			break;
		}
	}
