...
// Host-callable launcher for the batched distance kernels HnswIndexDist's
// copied search loop (hnsw_index_dist.cpp) uses to offload per-hop
...
// here, same reason HnswIndex::makeEngine() rejects it on the host side).
...
// hnsw_index_dist.cpp) because it's the one piece here that's actual
...
// problem entirely; HnswIndexDist::TraverseOneOnDevice() (hnsw_index_dist.cpp,
...
/// between `query` (one vector, `dim` elements of `dtype`, already
/// on-device) and each of `count` candidates, whose per-vector device
/// pointers are given by `candidate_ptrs` (itself a device array of
/// `count` pointers -- see hnsw_index_dist.cpp for how that's built, one
/// Region Lease per distinct Region the hop's candidates touch). `query`
/// and every pointer `candidate_ptrs` contains are typed as `dtype`
...
/// here so HnswIndexDist's host-side search loop, which already assumes
...
/// lifetime (query, candidate_ptrs, each candidate itself, out) lasting
/// until the launch completes -- e.g. by holding the relevant
/// gpu::DeviceRegionPool::Lease(s) and syncing/copying `out` back before
/// releasing them. Throws std::invalid_argument for DistanceMetric::Cosine
/// (not a real hnswlib Space -- see this file's own overview).
