...
이 타입들은 모든 레이어 경계(`Engine` -> `Core` -> `IndexAdapter`/`IRegion`)를
...
1. **Interface** -- `Index`, `Engine`. `Index`는 애플리케이션 코드가
   ...
   `Index` 구현체입니다. `IndexAdapter`와 `RoutingCache`를 각각
...
2. **Core** -- `Core`, `AnchorManager`. `Core`는 index에 대해 알지 못하는
   ...
   `IndexAdapter`/`IRegion` 인터페이스와 `RoutingCache` 인터페이스에
   대해서만 작성되어 있으며, 구체적인 index나 구체적인 cache 구현에
   대해서는 절대 작성되지 않습니다. `Core`가 보유하는 것:
   - `IndexAdapter& adapter_` -- 주입받으며, 소유하지 않습니다.
   ...
   `RoutingCache`나 `IndexAdapter`, 그 이상의 어떤 것도 알지 못하며,
   오직 `VectorId -> Stitch` 부기만을 이해합니다.

3. **Adapter** -- `IndexAdapter`, `IRegion` (인터페이스만 존재하며, 현재
